...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    train_image = x_train[i].reshape(-1)
    for j in range(num_test):
      test_image = x_test[j].reshape(-1)
      squared_euclidean_distance = torch.sum((train_image - test_image) ** 2).item()
      dists[i][j] = squared_euclidean_distance
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    train_image = x_train[i].reshape(-1)
    test_images = x_test.reshape(num_test, -1)
    squared_euclidean_distances = torch.sum((train_image - test_images) ** 2, dim=1)
    dists[i] = squared_euclidean_distances
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  train_images = x_train.reshape(num_train, -1)
  test_images = x_test.reshape(num_test, -1)

  sum_square_train_images = torch.sum(train_images ** 2, dim=1).view(num_train, 1)
  sum_square_test_images = torch.sum(test_images ** 2, dim=1).view(1, num_test)
  dists = sum_square_test_images \
          + sum_square_train_images \
          -2 * torch.mm(train_images, test_images.t())
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, k_indices = torch.topk(dists, k, dim=0, largest=False)
  k_closest_categoried = y_train[k_indices]
  y_pred, _ = torch.mode(k_closest_categoried, dim=0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    k_to_accuracies[k] = []
    for val_set in range(num_folds):
      folded_x_train = torch.cat([x_train_folds[i] for i in range(num_folds) if i != val_set])
      folded_y_train = torch.cat([y_train_folds[i] for i in range(num_folds) if i != val_set])
      x_val = x_train_folds[val_set]
      y_val = y_train_folds[val_set]
      classifier = KnnClassifier(folded_x_train, folded_y_train)
      k_to_accuracies[k].append(classifier.check_accuracy(x_val, y_val, k=k))
