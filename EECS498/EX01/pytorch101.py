...
def multiples_of_ten(start, stop):
  ...
  # Replace "pass" statement with your code
  import math
  # new_start is the first multiple of 10 larger/equal to new
  new_start = 10 * math.ceil(start / 10) 
  if new_start <= stop:
    x = torch.arange(new_start, stop, 10, dtype=torch.float64)
  else:
...
def zero_row_min(x):
  ...
  y[range(len(row_min_idxs)), row_min_idxs] = 0
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def normalize_columns(x):
  ...
  col_vars = torch.sqrt(torch.sum((x - col_means) ** 2, dim=0) / (M - 1))
  y = (x - col_means) / col_vars
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
