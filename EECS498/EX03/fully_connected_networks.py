...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    hidden, cache_layer_1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    scores, cache_layer_2 = Linear.forward(hidden, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    # calc grads using back-prop
    dhidden, dW2, db2 = Linear.backward(dscores, cache_layer_2)
    dx, dW1, db1 = Linear_ReLU.backward(dhidden, cache_layer_1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(1, len(layer_dims)):
      in_dim, out_dim = layer_dims[i-1], layer_dims[i]
      self.params['W{i}'.format(i=i)] = torch.empty(in_dim, out_dim, dtype=dtype, device=device).normal_(mean=0, std=weight_scale)
      self.params['b{i}'.format(i=i)] = torch.zeros(out_dim, dtype=dtype, device=device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
      hidden, cache = Linear_ReLU.forward(hidden, self.params['W{i}'.format(i=i)], self.params['b{i}'.format(i=i)])
      caches.append(cache)
      if self.use_dropout:
        hidden, cache = Dropout.forward(hidden, self.dropout_param)
        dropout_caches.append(cache)
    scores, cache = Linear.forward(hidden, self.params['W{i}'.format(i=self.num_layers)], self.params['b{i}'.format(i=self.num_layers)])
    caches.append(cache)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # calc softmax loss
    loss, dscores = softmax_loss(scores, y)
    loss += 0.5 * self.reg * sum([torch.sum(self.params['W{i}'.format(i=i)].pow(2)) for i in range(1, self.num_layers+1)])

    # calc grads of last layer using back-prop of Linear
    weight_key = 'W{i}'.format(i=self.num_layers)
    bias_key = 'b{i}'.format(i=self.num_layers)
    dhidden, grads[weight_key], grads[bias_key] = Linear.backward(dscores, caches[-1])
    grads[weight_key] += self.reg * self.params[weight_key]

    # calc grads of all other layers using back-prop of Linear_ReLU
    for i in range(self.num_layers - 1, 0, -1):
      if self.use_dropout:
        dhidden = Dropout.backward(dhidden, dropout_caches[i-1])
        caches.append(cache)
      weight_key = 'W{i}'.format(i=i)
      bias_key = 'b{i}'.format(i=i)
      dhidden, grads[weight_key], grads[bias_key] = Linear_ReLU.backward(dhidden, caches[i-1])
      grads[weight_key] += self.reg * self.params[weight_key]
...
def adam(w, dw, config=None):
  ...
  next_w = w - config['learning_rate'] * unbiased_m / (unbiased_v.sqrt() + config['epsilon'])
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = (torch.rand(x.shape, device=x.device) > p).int() / (1-p)
      out = x * mask
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
