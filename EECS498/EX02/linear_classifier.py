...
def svm_loss_vectorized(W, X, y, reg):
  ...
  scores = W.t().mm(X.t())
  correct_class_score = scores[y, torch.arange(scores.shape[1])]
  ...
  #############################################################################
  # Replace "pass" statement with your code
  add_to_dw = torch.sum(X, dim=0).repeat(num_classes, 1)

  A = torch.zeros((num_classes, num_train), dtype=X.dtype, device=X.device)
  A[y, range(A.shape[1])] = 1
  decrease_from_dw = num_classes * torch.mm(A, X)

  dW += add_to_dw.t()
  dW -= decrease_from_dw.t()
...
def predict_linear_classifier(W, X):
  ...
  y_pred = torch.argmax(scores, dim=0)
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
  return y_pred
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [10**(i/10) for i in range(-50, 0, 5)]
  regularization_strengths = [10**(i/10) for i in range(-50, 0, 5)]
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    correct_class_score = scores[y[i]]
    correct_class_probability = (torch.exp(correct_class_score)) / torch.sum(torch.exp(scores))
    image_loss = -torch.log(correct_class_probability)
    loss += image_loss
    
    dW_addition = (torch.exp(scores) / torch.sum(torch.exp(scores))).unsqueeze(1).mm(X[i].unsqueeze(0)) 
    dW_substraction = torch.zeros_like(dW.t())
    dW_substraction[y[i], range(dW_substraction.shape[1])] = X[i]
    dW += dW_addition.t() - dW_substraction.t()
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  correct_class_score = scores[y, torch.arange(scores.shape[1])]
  correct_class_probability = (torch.exp(correct_class_score)) / torch.sum(torch.exp(scores), dim=0)
  image_loss = -torch.log(correct_class_probability)
  loss += torch.sum(image_loss)
  
  coefficients = torch.exp(scores) / torch.sum(torch.exp(scores), dim=0)
  dW_addition = coefficients.mm(X)
  
  dW_substraction = torch.zeros(num_train, num_classes, X.shape[1], dtype=X.dtype, device=X.device)
  dW_substraction[torch.arange(num_train), y] = X
  dW_substraction = torch.sum(dW_substraction, dim=0)
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [10**(i/10) for i in range(-25, 10, 5)]
  regularization_strengths = [10**(i/10) for i in range(-50, -30, 5)]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
