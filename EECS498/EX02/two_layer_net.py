...
def nn_forward_pass(params, X):
    ...
    # Calc hidden
    hidden = torch.mm(X, W1) + b1.view(1, -1)
    hidden = torch.maximum(hidden, torch.zeros_like(hidden))

    # Calc scores
    scores = torch.mm(hidden, W2) + b2.view(1, -1)
...
def nn_forward_backward_not_enough_CPU(W1, W2, b1, b2, scores, h1, X, y, N, D, C, H, reg):
  grads = {}
  ...
  for i in range(N):
    dW2_addition = (torch.exp(scores[i]) / torch.sum(torch.exp(scores[i]))).unsqueeze(1).mm(h1[i].unsqueeze(0)) 
    dW2_substraction = torch.zeros_like(dW2_addition)
    dW2_substraction[y[i], range(dW2_substraction.shape[1])] = h1[i]
    grads['W2'] += dW2_addition.t() - dW2_substraction.t()

    coefficients = (torch.exp(scores[i]) / torch.sum(torch.exp(scores[i]))).view(-1, 1, 1).repeat(1, H, D)
    remove_relu_elements = (1 - (h1[i] == 0).int()).view(1, -1, 1).repeat(C, 1, D)
    W2_elements = W2.t().unsqueeze(2).repeat(1, 1, D)
    X_elements = X[i].repeat(C, H, 1)
    dW1_assist_mat = remove_relu_elements * W2_elements * X_elements
    dW1_addition = torch.sum(dW1_assist_mat * coefficients, dim=0)
    dW1_substraction = dW1_assist_mat[y[i]]
    grads['W1'] += dW1_addition.t() - dW1_substraction.t()

    coefficients = (torch.exp(scores[i]) / torch.sum(torch.exp(scores[i])))

    b2_addition = coefficients
    b2_substraction = torch.zeros_like(b2_addition)
    b2_substraction[y[i]] = 1
    grads['b2'] += b2_addition - b2_substraction
    b1_remove_relu_elements = (1 - (h1[i] == 0).int()).view(1, -1).repeat(C, 1)
    b1_coefficients = coefficients.view(-1, 1).repeat(1, H)
    b1_assist_mat = W2.t() * b1_remove_relu_elements
    b1_addition = torch.sum(b1_coefficients * b1_assist_mat, dim=0)
    b1_substraction = b1_assist_mat[y[i]]
    grads['b1'] += b1_addition - b1_substraction
...
def nn_forward_backward_not_enough_RAM(W1, W2, b1, b2, scores, h1, X, y, N, D, C, H, reg):
  grads = {}
  grads['W1'] = torch.zeros_like(W1)
  grads['W2'] = torch.zeros_like(W2)
  grads['b1'] = torch.zeros_like(b1)
  ...
  new_coefficients = (torch.exp(scores) / torch.sum(torch.exp(scores), dim=1).view(-1, 1))
  correct_classes = torch.zeros(N, C)
  correct_classes[torch.arange(N), y] = 1

  new_dW1_coefficients = new_coefficients.reshape(N, C, 1, 1).repeat(1, 1, H, D)
  new_dW1_remove_relu_elements = (1 - (h1 == 0).int()).reshape(N, 1, H, 1).repeat(1, C, 1, D)
  new_dW1_W2_elements = W2.t().reshape(1, C, H, 1).repeat(N, 1, 1, D)
  new_dW1_X_elements = X.reshape(N, 1, 1, D).repeat(1, C, H, 1)
  new_dW1_assist_mat = new_dW1_W2_elements * new_dW1_X_elements * new_dW1_remove_relu_elements
  new_dW1_addition = torch.sum(new_dW1_coefficients * new_dW1_assist_mat, dim=(0, 1))
  new_dW1_substraction = torch.sum(new_dW1_assist_mat[torch.arange(N), y, :, :], dim=0)
  grads['W1'] = new_dW1_addition.t() - new_dW1_substraction.t()
  
  new_dW2_addition = torch.matmul(new_coefficients.view(N ,C ,1), h1.view(N, -1, H))
  new_dW2_addition = torch.sum(new_dW2_addition, dim=0)
  new_dW2_substraction = correct_classes.view(N, C, 1).repeat(1, 1, H)
  new_dW2_substraction = torch.sum(new_dW2_substraction * h1.view(N, 1, H).repeat(1, C, 1), dim=0)
  grads['W2'] = new_dW2_addition.t() - new_dW2_substraction.t()

  new_b1_remove_relu_elements = (1 - (h1 == 0).int()).reshape(N, 1, H).repeat(1, C, 1)
  new_b1_coefficients = new_coefficients.reshape(N, C, 1).repeat(1, 1, H)
  new_b1_assist_mat = W2.t().reshape(1, C, H).repeat(N, 1, 1) * new_b1_remove_relu_elements
  new_b1_addition = torch.sum(new_b1_coefficients * new_b1_assist_mat, dim=(0, 1))
  new_b1_substraction = torch.sum(new_b1_assist_mat[torch.arange(N), y], dim=0)
  grads['b1'] += new_b1_addition - new_b1_substraction
  
  new_db2_addition = torch.sum(new_coefficients, dim=0)
  new_db2_substraction = torch.sum(correct_classes, dim=0)
  grads['b2'] = new_db2_addition - new_db2_substraction
...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    # shift to avoid numerical instability
    scores -= torch.max(scores)
    correct_class_score = scores[torch.arange(scores.shape[0]), y]
    correct_class_probability = (torch.exp(correct_class_score)) / torch.sum(torch.exp(scores), dim=1)
    image_loss = -torch.log(correct_class_probability)
    not_reg_loss = torch.sum(image_loss) / N
    loss = not_reg_loss + reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    C, H = W2.shape[1], h1.shape[1]
    
    #grads_not_enough_ram = nn_forward_backward_not_enough_RAM(W1, W2, b1, b2, scores, h1, X, y, N, D, C, H, reg)
    #grads_not_enough_cpu = nn_forward_backward_not_enough_CPU(W1, W2, b1, b2, scores, h1, X, y, N, D, C, H, reg)    
    
    exp_scores = torch.exp(scores)
    sum_exp_scores = torch.sum(exp_scores, axis=1)
    exp_correct_class_score = exp_scores[torch.arange(N), y]

    d_scores = torch.divide(exp_scores, sum_exp_scores.reshape(N, 1)) / N
    d_scores[torch.arange(N), y] = (exp_correct_class_score - sum_exp_scores) / (N * sum_exp_scores)
    d_b2 = torch.sum(d_scores, axis=0)
    d_W2 = torch.mm(h1.t(), d_scores)
    d_h = torch.mm(d_scores, W2.t())
    # z = hidden before ReLU -> h = max(z, 0)
    d_z = d_h * (h1 > 0).int()
    d_b1 = torch.sum(d_z, axis=0)
    d_W1 = torch.mm(X.t(), d_z)
...
def nn_get_search_params():
  ...
  regularization_strengths = [0, 1e-5, 1e-4, 1e-2]
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  best_net, best_stat, best_val_acc = None, None, None
    ...
    for hs in hidden_sizes:
      for reg in regularization_strengths:
        for lrd in learning_rate_decays:
          print("Training network with: learning rate = {lr}; hidden size = {hs}; regularization = {reg};".format(lr=lr, hs=hs, reg=reg, lrd=lrd))
          ...
          stats = net.train(data_dict['X_train'], data_dict['y_train'], data_dict['X_val'], data_dict['y_val'], 
                            num_iters=3000, batch_size=1000, verbose=False,
                            learning_rate=lr, learning_rate_decay=lrd, reg=reg)
          val_acc = stats['val_acc_history'][-1]
          
          if (best_net is None) or (best_stat is None) or (best_val_acc is None) or (val_acc > best_val_acc):
            best_net, best_stat, best_val_acc = net, stats, val_acc
